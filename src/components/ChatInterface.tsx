...
export default function ChatInterface({ 
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ 
          message: content,
          sessionId: sessionId // Pass the sessionId to maintain conversation context
        })
      })
      ...
      const aiMessage: Message = {
        ...
        sessionId,
        timestamp: new Date().toISOString(),
        intent: data.intent, // Store the detected intent
        confidence: data.confidence // Store the confidence score
      }
  ...
  // Add handler functions for feedback
                  ...
                  <div className="flex flex-col">
                    ...
                    {message.role === 'assistant' && (
                      <div className="flex gap-2 mt-1 ml-2">
                        ...
                        >
                          <ThumbsUp 
                            className={`w-4 h-4 ${feedback[message.id]?.thumbsUp ? 'text-[#007d8c] fill-[#007d8c]' : 'text-gray-500'}`} 
                          />
                        </button>
                        <button 
                        ...
                        >
                          <ThumbsDown 
                            className={`w-4 h-4 ${feedback[message.id]?.thumbsDown ? 'text-[#007d8c] fill-[#007d8c]' : 'text-gray-500'}`} 
                          />
                        </button>
                      </div>
                    )}
